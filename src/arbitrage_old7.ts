  ...
  function calculatePathWeight(g: Graph, cycle: string[]): { cycleWeight: number, detailedCycle: any[] } {
    ...
    for (let index = 0; index < cycle.length - 1; index++) {
      ...
      const feeTier = edge.metadata.feeTier ? Number(edge.metadata.feeTier) / 1000000 : 0.003;
      const effectiveWeight = edge.rawWeight * (1 - feeTier);

      cycleWeight *= effectiveWeight;

      let transactionType = edge.rawWeight > 1 ? 'buy' : 'sell';
  ...
  async function calcArbitrage(g: Graph): Promise<ArbitrageRoute[]> {
    ...
    for (const vertex of g.getAllVertices()) {
      ...
      for (var cycle of cyclePaths) {
        if (cycle.length >= 3  && cycle.length <= 11) { // 4 y 11 porque el primer y último elemento son el mismo
          let cycleString = cycle.join('');
          let { cycleWeight, detailedCycle } = calculatePathWeight(g, cycle);
          let feeTotal = 0;
          for (const step of detailedCycle) {
            feeTotal += step.feeTier ? Number(step.feeTier) / 1000000 : 0.003;
          }
          if (!uniqueCycle[cycleString] && cycleWeight >= 1 + MINPROFIT * (1 - feeTotal)) {
  ...
  async function calculateRouteProfit(route: ArbitrageRoute, amount: string): Promise<{ profit: string, profitWithFees: string }> {
    ...
    if (steps.length < 3 || steps.length > 10) {
      console.warn(`Ruta inválida: ${steps.length} pasos`);
      return { profit: '0', profitWithFees: '0' };
    }

    for (const step of steps) {
      try {
        if (step.dex === DEX.UniswapV3 || step.dex === DEX.Sushiswap) {
          const { price } = step.dex === DEX.UniswapV3 
            ? await getUniswapV3PoolData(step.poolAddress)
            : await getSushiswapPoolData(step.poolAddress);
          
          const feeTier = step.feeTier ? Number(step.feeTier) / 1000000 : 0.003; // 0.003 para Sushiswap
          const effectivePrice = step.type === "buy" ? price * (1 + feeTier) : price * (1 - feeTier);
          
          if (step.type === "buy") {
            currentAmount = JSBI.divide(
              JSBI.multiply(currentAmount, JSBI.BigInt(1e18)),
              JSBI.BigInt(Math.floor(effectivePrice * 1e18))
            );
          } else {
            currentAmount = JSBI.divide(
              JSBI.multiply(currentAmount, JSBI.BigInt(Math.floor(effectivePrice * 1e18))),
              JSBI.BigInt(1e18)
            );
          }
        }
    ...
    // Aplicamos el LENDING_FEE solo una vez al final
    const lendingFeeAmount = JSBI.divide(
      JSBI.multiply(initialAmount, JSBI.BigInt(Math.floor(LENDING_FEE * 1e18))),
      JSBI.BigInt(1e18)
    );
    const profitWithFees = JSBI.subtract(profit, lendingFeeAmount);

    // Aplicamos el SLIPPAGE al profit final
    const profitWithSlippage = JSBI.subtract(
      profitWithFees,
      JSBI.divide(
        JSBI.multiply(profitWithFees, JSBI.BigInt(Math.floor(SLIPPAGE * 1e18))),
        JSBI.BigInt(1e18)
      )
    );

    return {
      profit: ethers.formatUnits(profit.toString(), 'gwei'),
      profitWithFees: ethers.formatUnits(profitWithSlippage.toString(), 'gwei')
    };
  }
  async function processArbitrageRoutes(routes: ArbitrageRoute[]): Promise<ArbitrageRoute[]> {
    ...
    for (const route of routes) {
      try {
        ...
        const { profit, profitWithFees } = await calculateRouteProfit(route, montomaxflashloan);
        const isRentable = JSBI.greaterThan(
          JSBI.BigInt(ethers.parseUnits(profitWithFees, 'gwei').toString()),
          JSBI.BigInt(0)
        );
        ...
        processedRoutes.push({
          ...
          estimatedProfitWithFees: profitWithFees,
          isRentable
        });
      } catch (error) {
