interface Pool {
  ...
  export function verfiy_token_path(path: Pool[]): void {
  ...
  export function uniswap_V3_swap_math(pool: Pool, amount: number): number {
    ...
    function calc_amount0(liq: number, pa: number, pb: number): number {
      if (pa > pb) {
        [pa, pb] = [pb, pa];
      }
    ...
    function calc_amount1(liq: number, pa: number, pb: number): number {
      if (pa > pb) {
        [pa, pb] = [pb, pa];
      }
  ...
  export function uniswap_V2_sushiswap_swap_math(pool: Pool, amount: number): number {
