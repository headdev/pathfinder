  ...
  interface ArbitrageRoute {
    ...
    detail: string;
    type: string;
    dexPath: DEX[]; // Cambiado de string[] a DEX[]
  }
    ...
    for (const sourceVertex of sourceVertices) {
      ...
      for (const targetVertex of targetVertices) {
        if (distances[targetVertex.getKey()] < 0) {
          ...
          const dexPath: DEX[] = arbitragePath.map((_, index) => {
            if (index === arbitragePath.length - 1) return DEX.UniswapV3; // Placeholder, adjust as needed
            const edge = graph.findEdge(graph.getVertexByKey(arbitragePath[index]), graph.getVertexByKey(arbitragePath[index + 1]));
            return edge ? edge.metadata.dex : DEX.UniswapV3; // Use the DEX enum
          });
          const arbitrage: ArbitrageRoute = {
            ...
            detail: arbitragePath.map((token, index) => `${token} (${DEX[dexPath[index]]})`).join(' -> '),
            type: 'non-cyclic',
            dexPath: dexPath
          };
    ...
    for (const originToken of ORIGIN_TOKENS) {
      ...
      for (const cycle of cycles) {
        ...
        if (!uniqueCycle[cycleString]) {
          ...
          // Ajustamos el umbral de rentabilidad para tener en cuenta todos los costos
  ...
  function calculateCycleWeight(g: Graph, cycle: string[]): { weight: number, dexPath: DEX[] } {
    ...
    for (let i = 0; i < cycle.length - 1; i++) {
      const edge = g.findEdge(g.getVertexByKey(cycle[i]), g.getVertexByKey(cycle[i + 1]));
      if (edge) {
        // Incluimos el fee de la pool y el slippage
    ...
    // Aplicamos el fee del préstamo flash al final del ciclo
